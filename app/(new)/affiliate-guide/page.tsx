    ...
    <PageWrapper ctaTitle="Ready to Start Earning?" ctaSubtitle="Join the Orchard Capitals Affiliate Program today and start earning commissions on every referral.">
      ...
      <section style={{ background: WHITE, padding: "88px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ maxWidth: 640, marginBottom: 52 }}>
            ...
            <motion.h2 {...reveal} style={{ fontFamily: "var(--oc-poppins)", fontSize: "clamp(28px, 3.5vw, 44px)", fontWeight: 400, lineHeight: 1.1, letterSpacing: "-0.5px", marginBottom: 12 }}>
              Up and running in<br /><em style={{ color: OC, fontStyle: "italic" }}>four steps.</em>
            </motion.h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {GETTING_STARTED.map((step, i) => (
              <motion.div key={i} {...reveal} style={{ display: "flex", gap: 20, alignItems: "flex-start", background: GRAY, borderRadius: 14, padding: "20px 24px", border: "1px solid #e5e7eb" }}>
          ...
          <div style={{ display: "grid", gap: 40 }} className="grid-cols-1 lg:grid-cols-2">
            <div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 100, padding: "5px 12px", marginBottom: 20 }}>
                <span style={{ color: WHITE, fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>Your Dashboard</span>
              </div>
              <motion.h2 {...reveal} style={{ fontFamily: "var(--oc-poppins)", fontSize: "clamp(24px, 3vw, 36px)", fontWeight: 400, lineHeight: 1.1, marginBottom: 24, letterSpacing: "-0.5px", color: WHITE }}>
                Your command<br /><em style={{ fontStyle: "italic" }}>center.</em>
              </motion.h2>
              {DASHBOARD_FEATURES.map(f => (
                <motion.div key={f} {...reveal} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: WHITE, flexShrink: 0, marginTop: 8 }} />
                  <span style={{ color: "rgba(255,255,255,0.85)", fontSize: 15, lineHeight: 1.7 }}>{f}</span>
                </motion.div>
              ))}
            </div>
            <div>
      ...
      <section style={{ background: WHITE, padding: "88px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <motion.h2 {...reveal} style={{ fontFamily: "var(--oc-poppins)", fontSize: "clamp(28px, 4vw, 44px)", fontWeight: 400, lineHeight: 1.1, letterSpacing: "-0.5px", marginBottom: 12 }}>
              Tips to maximize<br /><em style={{ color: OC, fontStyle: "italic" }}>your earnings.</em>
            </motion.h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {PROMOTION_TIPS.map(tip => (
              <motion.div key={tip.title} {...reveal} style={{ display: "flex", gap: 16, alignItems: "flex-start", background: GRAY, borderRadius: 14, padding: "20px 24px", border: "1px solid #e5e7eb" }}>
      ...
      <section style={{ background: GRAY, padding: "88px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ display: "grid", gap: 40 }} className="grid-cols-1 lg:grid-cols-2">
            <div>
              <motion.h2 {...reveal} style={{ fontFamily: "var(--oc-poppins)", fontSize: "clamp(24px, 3vw, 36px)", fontWeight: 400, lineHeight: 1.1, marginBottom: 24, letterSpacing: "-0.5px" }}>
                Comprehensive<br /><em style={{ fontStyle: "italic", color: OC }}>tracking and analytics.</em>
              </motion.h2>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {TRACKING.map(t => (
                  <motion.div key={t.label} {...reveal} style={{ background: WHITE, borderRadius: 12, padding: "18px 20px", border: "1px solid #e5e7eb" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {FAQS.map(faq => (
                  <motion.div key={faq.q} {...reveal} style={{ background: WHITE, borderRadius: 12, padding: "18px 20px", border: "1px solid #e5e7eb" }}>

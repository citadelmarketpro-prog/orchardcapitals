    ...
    <PageWrapper ctaTitle="Start Earning with Orchard Capitals" ctaSubtitle="Join our affiliate program and earn 10% commission on every qualified referral — no cap, no delay.">
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ display: "grid", gap: 1, background: "rgba(0,0,0,0.06)", borderRadius: 16, overflow: "hidden" }} className="grid-cols-2 sm:grid-cols-4">
            {[["10%", "Commission Per Referral"], ["90 Days", "Cookie Duration"], ["$0", "Minimum Withdrawal"], ["Unlimited", "Referrals Allowed"]].map(([val, lbl]) => (
              <div key={lbl} style={{ padding: "40px 24px", background: WHITE, textAlign: "center" }}>
      ...
      <section style={{ background: WHITE, padding: "88px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gap: 24 }} className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map(s => (
              <div key={s.num} style={{ background: GRAY, borderRadius: 16, padding: "32px 24px", border: "1px solid #e5e7eb" }}>
      ...
      <section style={{ background: GRAY, padding: "88px 24px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 60 }}>
            <h2 style={{ fontFamily: "var(--oc-poppins)", fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 400, lineHeight: 1.1, letterSpacing: "-0.5px", marginBottom: 12 }}>
              Everything you get as an<br /><em style={{ color: OC, fontStyle: "italic" }}>Orchard Capitals affiliate.</em>
            </h2>
          </div>
          <div style={{ display: "grid", gap: 20 }} className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {BENEFITS.map(b => (
              <div key={b.title} style={{ background: WHITE, borderRadius: 16, padding: "28px 24px", border: "1px solid #e5e7eb" }}>
      ...
      <section style={{ background: WHITE, padding: "88px 24px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ marginBottom: 40 }}>
            <h2 style={{ fontFamily: "var(--oc-poppins)", fontSize: "clamp(24px, 3vw, 36px)", fontWeight: 400, lineHeight: 1.1, letterSpacing: "-0.5px", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {RULES.map(rule => (
              <div key={rule} style={{ display: "flex", gap: 14, alignItems: "flex-start", background: GRAY, borderRadius: 12, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
